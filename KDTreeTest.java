...
public class KDTreeTest extends TestCase {
  ...
  * The KDTree instance used for testing */
  ...
  /**
   ...
   public void testPrintComprehensive() {
     ...
     assertEquals("2    Baltimore (0, 300)\n"
        + "3      Washington (5, 350)\n1  Atlanta (10, 500)\n"
         + "1  Tacoma (1000, 100)\n2    L (101, 150)\n",
         tree.printTree());
   }
   ...
   */
    ...
    tree.insert(new City("UpExact", 50, 60));
     ...
     assertEquals("1  LeftExact (40, 50)\n0Root (50, 50)\n"
         + "2    DownExact (50, 40)\n1  RightExact (60, 50)\n"
         + "2    UpExact (50, 60)\n", tree.printTree());
   }
  ...
  public void testTreeStructureIntegrity() {
     ...
     assertEquals("2    LeftLeftY (20, 30)\n"
        + "1  LeftX (25, 60)\n2    LeftRightY (30, 70)\n"
         tree.printTree());
   }
     ...
     String[] parts = result.split("\n");
      assertEquals(2, parts.length);
       assertTrue(parts[0].matches("\\d+"));
        ...
        assertEquals("0Replacement (60, 40)\n",
            tree.printTree());
   ...
   */
    ...
    assertEquals("2    LessXLessY (30, 40)\n"
          + "1  LessXMoreY (40, 60)\n0Center (50, 50)\n",
          tree.printTree());
    }
   ...
   * When query region intersects both sides, both must be
   ...
   public void testSearchBothSubtrees() {
    ...
    /**
      ...
      * */
        est
      u  tree.insert(new City("Root", 50, 50));
   ...
   * (dx) for pruning during range searches. Ensures that nodes
   ...
   public void testSearchDifferenceCalculationXAxis() {
    // Set up a tree where X-axis pruning matters
     ...
     assertEquals("1  LeftChild (50, 60)\n"
          + "2    FarLeft (20, 70)\n0Root (100, 50)\n"
          + "2    FarRight (180, 30)\n"
          + "1  RightChild (150, 40)\n", tree.printTree());
  ...
  @Test
    ...
    /**
   * Tests search boundary condition where diff > -radius is FALSE.
     * This kills the mutation on line 417 that replaces the condition with true.
   * The left subtree should NOT be visited when diff <= -radius.
     */
    @Test
    public void testSearchLeftBoundaryExcluded() {
      // Build a simple tree with root at (100, 100)
    tree.insert(new City("Root", 100, 100));
      tree.insert(new City("Left", 50, 100)); // Left child, 50 units left
     tree.insert(new City("Right", 150, 100)); // Right child, 50 units right

     // Root is at x=100, query is at x=140, so diff = 100 - 140 = -40
     // Since diff (-40) is NOT > -radius (-5), left subtree should NOT be visited
     // Only root and right child should be checked
     String result = tree.search(140, 100, 5);
  
     // Extract visit count (last line of result)
     String[] lines = result.split("\n");
     int visitCount = Integer.parseInt(lines[lines.length - 1].trim());
  
      // Should visit: Root + Right = 2 nodes (NOT Left)
      assertEquals(2, visitCount);
    }
    
    /**
     * Tests search boundary condition where diff <= radius is FALSE.
   * This kills the mutation on line 419 that replaces the condition with true.
     * The right subtree should NOT be visited when diff > radius.
     */
    @Test
    public void testSearchRightBoundaryExcluded() {
    // Build a simple tree with root at (100, 100)
      tree.insert(new City("Root", 100, 100));
    tree.insert(new City("Left", 50, 100)); // Left child
      tree.insert(new City("Right", 150, 100)); // Right child, 50 units right
    
      // Search from point (60, 100) with radius 5
      // Root is at x=100, query is at x=60, so diff = 100 - 60 = 40
    // Since diff (40) is NOT <= radius (5), right subtree should NOT be visited
      // Only root and left child should be checked
     String result = tree.search(60, 100, 5);

     String[] lines = result.split("\n");
     int visitCount = Integer.parseInt(lines[lines.length - 1].trim());
  
     // Should visit: Root + Left = 2 nodes (NOT Right)
     assertEquals(2, visitCount);
   }
  
   /**
    * Tests search where query point requires exploring subtrees at
     * alternating depths with correct axis calculations.
     * Ensures depth+1 is computed correctly for both left (line 418)
     * and right (line 420) recursive calls.
     */
    @Test
    public void testSearchAlternatingAxisDepth() {
    // Create a tree with specific structure
      tree.insert(new City("A", 100, 100)); // depth 0, X-axis
      tree.insert(new City("B", 50, 150)); // depth 1, Y-axis (left of A)
      tree.insert(new City("C", 150, 50)); // depth 1, Y-axis (right of A)
      tree.insert(new City("D", 25, 150)); // depth 2, X-axis (left of B on Y)
    tree.insert(new City("E", 75, 150)); // depth 2, X-axis (right of B on Y)
      tree.insert(new City("F", 150, 25)); // depth 2, X-axis (left of C on Y)
    tree.insert(new City("G", 150, 75)); // depth 2, X-axis (right of C on Y)
    
      // Search that will explore multiple depths on both sides
      // Query at (100, 100) with radius 60
      String result = tree.search(100, 100, 60);

      String[] lines = result.split("\n");
     int visitCount = Integer.parseInt(lines[lines.length - 1].trim());

     // The exact count depends on pruning, but we verify results are correct
     assertTrue(visitCount >= 4); // At minimum, explores multiple branches
     assertTrue(result.contains("A")); // Root is in range
   }
  
   // ======================
   /**
    * Tests that depth is correctly incremented when recursing left (line 418).
     * The visit count must be exact to catch when depth+1 is mutated to 'depth' or
     * '1'.
     * Incorrect depth causes wrong axis selection, leading to different pruning.
     */
  @Test
    public void testSearchLeftRecursionDepthIncrement() {
      // Build a tree where axis selection matters for pruning
      tree.insert(new City("A", 100, 100)); // depth 0, X-axis
    tree.insert(new City("B", 50, 150)); // depth 1, Y-axis (left on X)
      tree.insert(new City("C", 25, 150)); // depth 2, X-axis (left on Y)
    tree.insert(new City("D", 75, 150)); // depth 2, X-axis (right on Y)
      tree.insert(new City("E", 150, 100)); // depth 1, Y-axis (right on X)
    
      // Search that explores left subtree with careful pruning
    // Query at (40, 150) with radius 30
      // This should visit A, B, C (and prune correctly based on axis)
     String result = tree.search(40, 150, 30);

     // Parse the result to check visit count
     String[] lines = result.split("\n");
     String lastLine = lines[lines.length - 1];
     int visitCount = Integer.parseInt(lastLine.trim());
  
     // Build expected output string with exact visit count
     // When depth is correct: visits A, B, C, D (4 nodes)
   // When depth+1 becomes 'depth': axis doesn't change, wrong pruning
   // When depth+1 becomes '1': axis is always Y after root, wrong pruning
 ...
 /**
Tests that depth is correctly incremented when recursing right (line 420).
  * The visit count must be exact to catch when depth+1 is mutated to 'depth' or
  * '1'.
  * Incorrect depth causes wrong axis selection, leading to different pruning.
 ...
 public void testSearchRightRecursionDepthIncrement() {
     // Build a tree where axis selection matters for pruning
    tree.insert(new City("A", 100, 100)); // depth 0, X-axis
     tree.insert(new City("B", 50, 100)); // depth 1, Y-axis (left on X)
     tree.insert(new City("C", 150, 150)); // depth 1, Y-axis (right on X)
     tree.insert(new City("D", 150, 125)); // depth 2, X-axis (left on Y)
     tree.insert(new City("E", 150, 175)); // depth 2, X-axis (right on Y)
  
     // Search that explores right subtree with careful pruning
     // Query at (160, 150) with radius 30
   String result = tree.search(160, 150, 30);

   // Parse and verify exact visit count
   String[] lines = result.split("\n");
tring lastLine = lines[lines.length - 1];
   int visitCount = Integer.parseInt(lastLine.trim());

   // Expected: visits A, C, D (3 nodes) and finds C, D
   // Mutation would cause different visit count due to wrong axis pruning
   String expected = "C (150, 150)\n" +
   "D (150, 125)\n" +
       "E (150, 175)\n" +
       "4";

ssertEquals(expected, result);
 }

   /**
   * Tests mutation coverage for KDTree deleteRec - coordinate equality checks
    */
   @Test
   public void testDeleteCoordinateMatchingBothAxis() {
     KDTree tree = new KDTree();
  
     // Insert cities strategically so coordinate checks matter
     City cityA = new City("CityA", 50, 50); // root
     City cityB = new City("CityB", 25, 75); // different x and y from target
   City cityC = new City("CityC", 50, 75); // same x as A, different y
   City cityD = new City("CityD", 25, 50);// different x, same y as A

   assertTrue(tree.insert(cityA));
   assertTrue(tree.insert(cityB));
   assertTrue(tree.insert(cityC));
   assertTrue(tree.insert(cityD));

/ Delete cityB at (25, 75)
   String result = tree.delete(25, 75);
   assertTrue(result.contains("CityB"));
   assertNull(tree.find(25, 75));

   // CRITICAL: Verify other cities are NOT deleted
   // If getX()==x replaced with true, wrong cities with x=25 might be affected
/ If getY()==y replaced with true, wrong cities with y=75 might be affected
   assertNotNull(tree.find(50, 50));
   ...
   *
     // Additional verification: delete another city and ensure precise targeting
     result = tree.delete(50, 75);
     assertTrue(result.contains("CityC"));
     assertNull(tree.find(50, 75));

   // CityA with same x=50 should still ext
   assertNotNull(tree.find(50, 50));
   assertEquals("CityA", tree.find(50, 50etName());
 }
